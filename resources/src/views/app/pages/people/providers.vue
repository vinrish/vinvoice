  ...
  <div class="main-content">
    ...
    <validation-observer ref="Create_Provider">
      <b-modal hide-footer size="lg" id="New_Provider" :title="editmode?$t('Edit'):$t('Add')">
        <b-form @submit.prevent="Submit_Provider" enctype="multipart/form-data">
          <b-row>
            <!-- Provider Name -->
            <b-col md="6" sm="12">
              <validation-provider
  ...
  methods: {
    ...
    Get_Providers(page) {
      ...
      async onFileSelected(e) {
          ...
          if (valid && e.target.files && e.target.files[0]) {
              this.provider.avatar = e.target.files[0];
              console.log('File selected:', this.provider.avatar);  // Add this line for debugging
          } else {
              this.provider.avatar = "";
              console.log('No file selected or file is invalid.');  // Add this line for debugging
          }
      },
    ...
    Create_Provider() {
      this.SubmitProcessing = true;
        ...
        self.data.append("name", self.provider.name);
        self.data.append("email", self.provider.email);
        self.data.append("phone", self.provider.phone);
        self.data.append("tax_number", self.provider.tax_number);
        self.data.append("country", self.provider.country);
        self.data.append("city", self.provider.city);
        self.data.append("adresse", self.provider.adresse);
        self.data.append("avatar", self.provider.avatar);

        self.data.append("_method", "post");

        axios.post("providers", self.data)
            .then(response => {
                Fire.$emit("Event_Provider");
                this.makeToast("success", this.$t("Create.TitleSupplier"), this.$t("Success"));
    ...
    Update_provider() {
      ...
      axios
        .post("providers/" + this.provider.id, self.data)
        .then(response => {
          Fire.$emit("Event_Provider");
